...
export async function groupMiddleware(command: string, m: any, sock: WASocket) {
  if (!m.key.remoteJid?.endsWith('g.us')) {
    return await sock.sendMessage(
      m.key.remoteJid as string,
      {
        text: `\`!${command}\` tidak berfungsi jika tidak di grup`,
...
export async function antilinkMiddleware(m: any, sock: WASocket) {
  ...
  if (status.antilink && /(http|https)/g.test(body))
    return await sock.sendMessage(m.key.remoteJid as string, {
      delete: m.key,
    })
}

export async function participantsUpdateMiddleware(
  m: any,
  sock: WASocket,
  command: string,
) {
